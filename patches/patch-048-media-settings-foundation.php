...
/**
 * TreeForge CMS - Patch 048
 * Media Settings Foundation - FIXED
 *
 * Wichtig:
 * Diese korrigierte Version ersetzt die vorherige patch-048-media-settings-foundation.php.
 *
 * Fix:
 * - Kein direkter Aufruf von TreeForge\Core\Settings\SettingsManager innerhalb des Patches.
 * - Dadurch kein Autoload-Problem im Patch-Runner.
 */
...
return function (string $root, callable $log): void {
    ...
    $log('Patch 048 Media Settings Foundation FIXED gestartet');
...
PHP;
    ...
    if (file_exists($settingsJsonFile)) {
        $decoded = json_decode((string)file_get_contents($settingsJsonFile), true);
        if (is_array($decoded)) {
            $current = $decoded;
        }
    }
    ...
    $write($root . '/docs/treeforge/38-media-settings-foundation.md', <<<'MD'
# Media Settings Foundation

Patch 048 ergänzt umfangreiche Media Settings.

## Wichtig

Diese Version ist korrigiert und ruft im Patch selbst keine frisch geschriebene Klasse auf.
...
MD);
    ...
    $log('Patch 048 Media Settings Foundation FIXED fertig');
};
