...
/**
 * TreeForge CMS - Patch 015
 * Archive Explorer + Rollback
 *
 * - ergänzt ArchiveManager
 * - normalisiert neues Archivformat zusätzlich zu alten Zeitstempel-Ordnern
 * - zeigt Archivversionen im Explorer
 * - ermöglicht Restore einer Archivversion nach Published
 */
...
return function (string $root, callable $log): void {
    ...
    $log('Patch 015 Archive Explorer + Rollback gestartet');

    $write($root . '/app/Core/ArchiveManager.php', <<<'PHP'
<?php
declare(strict_types=1);
...
PHP);
    ...
    $write($root . '/public/api/archive/restore.php', <<<'PHP'
<?php
declare(strict_types=1);
...
.tf-explorer-shell {
  display: grid;
  grid-template-columns: 260px minmax(300px, 1fr) minmax(420px, .95fr);
  gap: 1rem;
  padding: 1rem;
}
...
.tf-archive-link:hover,
.tf-archive-link.active {
...
.tf-workflow-link.secondary {
  background: #fff;
  color: var(--tf-green);
  border: 1px solid rgba(30, 61, 28, .18);
}

.tf-workflow-button.danger {
  background: #8a3b14;
}
...
CSS);
    ...
    $write($root . '/docs/archive-rollback.md', <<<'MD'
# Archive & Rollback
...
MD);
    ...
    $log('Patch 015 Archive Explorer + Rollback fertig');
};
