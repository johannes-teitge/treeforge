...
/**
 * TreeForge CMS - Patch 058
 * Fix Media Edit Replace Parse Error
 *
 * Fix:
 * public/admin/media/edit.php wird vollständig und sauber neu geschrieben.
 */
...
return function (string $root, callable $log): void {
    ...
    $log('Patch 058 Fix Media Edit Replace Parse Error gestartet');
...
$preview = $isImage
    ? '<img src="' . e($url) . '?v=' . e((string)($item['updated_at'] ?? time())) . '" alt="' . e((string)($item['alt'] ?? '')) . '">'
...
PHP);
    ...
    $write($root . '/docs/treeforge/48-fix-media-edit-replace-parse-error.md', <<<'MD'
# Fix Media Edit Replace Parse Error

Patch 058 repariert `public/admin/media/edit.php`.
...
MD);
    ...
    $log('Patch 058 Fix Media Edit Replace Parse Error fertig');
};
